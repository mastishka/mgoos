<?php
	...
	$database="ali";
	$db_link_id12 = mysql_connect('localhost','root','');
	...
	function InsertIntoMp3Info(CId3Info $id3info)
	{
		...
		//$db_link_id12 = mysql_connect('localhost','root','');
		//mysql_select_db($database, $db_link_id12);
		//mysql_query("insert into mp3_info (id) values ('$id')",$db_link_id12) or die("Sql insert error :- ".mysql_error($db_link_id12));


		//echo "insert ignore into mp3_data (id, upload_date, filepath, filesize, bitrate, duration_sec, language, status, user_id, title, artist, album, lyrics, year, genre,mood, composer, picturizedon, provider) values ('$id', CURDATE(), '$filepath', '$filesize', '$bitrate', '$duration_sec', '$language', $status, '$userid', '$title', '$artist', '$album', '$lyrics', '$year', '$genre','$mood', '$composer', '$picturizedon', '$provider');<br/>";

		
		$bResult = mysql_query("insert into mp3_data (id, upload_date, filepath, filesize, bitrate, duration_sec, language, status, user_id, title, artist, album, lyrics, year, genre,mood, composer, picturizedon, provider) values ('$id', SYSDATE(), '$filepath', '$filesize', '$bitrate', '$duration_sec', '$language', $status, '$userid', '$title', '$artist', '$album', '$lyrics', '$year', '$genre','$mood', '$composer', '$picturizedon', '$provider') ;") or die("Sql insert error :- ".mysql_error());
	}
	
	
	$links=mysql_query("select url from mp3 where satus=0",$db_link_id12) or die("Sql select error :- ".mysql_error($db_link_id12));	

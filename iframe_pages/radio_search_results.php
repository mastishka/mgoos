...
<HTML>
	...
	<BODY >
				...
				<TD VALIGN="TOP">
					<?php
						...
						if(isset($loadpop))
						{
							...
							$recordFound = $objDB->showPopularRadio($pg, CSessionManager::GetUserId()) ;
						}
						elseif(isset($last_week_pop))
						{
							$feature_type = 3; // last week popular has feature type = 3;
							$recordFound = $objDB->showLastWeekPopularRadio($pg, CSessionManager::GetUserId()) ;
						}
						//new
						elseif(isset($radio_playlist))
						{
							//feature type yet to be decided
							$feature_type = 5; // radio playlist has feature type = 5;
							$recordFound = $objDB->showRadioPlaylist($pg, CSessionManager::GetUserId()) ;
						}
						//end new
						elseif(isset($recent_uploads))
						{
							$feature_type = 4; // Recent upload has feature type = 4;
							$recordFound = $objDB->showRecentUploadsRadio($pg, CSessionManager::GetUserId()) ;
						}

						elseif (isset($browse_album))
						{
								$feature_type = 2 ; // browse album has feature type = 2
								$recordFound = $objDB->browseAlbumRadio($pg, CSessionManager::GetUserId(),$pageno) ;
						}
						else
					 	{
							...
							{
								$strict = 1 ;
								$recordFound = $objDB->listExactRadio($qry, $pg, $ext, CSessionManager::GetUserId()) ;
							}
							else
							{
								$strict = 0 ;
								// Now $qry will contain search string and $pg will contain page number.
								$recordFound = $objDB->showUptoTenRecordsRadio($qry, $pg, $ext, CSessionManager::GetUserId()) ;
							}
						}
					//	}
		...
		<?php
			...
			printf ("TSR.url = \"iframe_pages/radio_search_results.php\";\n");

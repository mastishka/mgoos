	...
	class CQueryManager
	{
		...
		public function InsertIntoMP3PlayLocation($mp3_id, $ip)
		{
		 ...
		 public function MakeProperEntries()
		 {
			...
			public function ListAlphabetRadio($alphabet, $columns, $tabbed = false)
			{
				$result = mysql_query("select distinct album from mp3_info where album like '".$alphabet."%' and provider <> 'http://www.mgoos.com'") or die("mp3_info select error: ".mysql_error($this->db_link_id));
				...
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					if($tabbed)
					{
						$url = "radio_search_results.php?qry=".urlencode($row["album"])."&pg=1&ext=album&strict=true" ;
					}
					else
					{
						$url = "internet_radio.php?srchtxt=".urlencode($row["album"])."&ext=album&strict=true" ;
					}
			...
			public function NormalizeMP3MetaphoneInfo()
			{
				$bResult = mysql_query("create table mp3_metaphone_info_1 as select mp3_metaphone_info.* from mp3_metaphone_info, mp3_info where mp3_info.id = mp3_metaphone_info.id") or die("create table mp3_metaphone_info_1 error: ".mysql_error($this->db_link_id));
				
				$bResult = mysql_query("drop table mp3_metaphone_info;") or die("drop table mp3_metaphone_info error: ".mysql_error($this->db_link_id));
				
				$bResult = mysql_query("rename table mp3_metaphone_info_1 to mp3_metaphone_info;") or die("rename table mp3_metaphone_info_1 error: ".mysql_error($this->db_link_id));
				
				$bResult = mysql_query("update mp3_info set title = 'Unknown' where title = ''") or die("update mp3_info set title error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_info set artist = 'Unknown' where artist = ''") or die("update mp3_info set artist error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_info set album = 'Unknown' where album = ''") or die("update mp3_info set album error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_info set album = REPLACE(binary album,'\0','')") or die("update mp3_info set album 2 error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_metaphone_info set title = 'UNKNN' where title = ''") or die("update mp3_metaphone_info set title error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_metaphone_info set artist = 'UNKNN' where artist = ''") or die("update mp3_metaphone_info set artist error: ".mysql_error($this->db_link_id));

				$bResult = mysql_query("update mp3_metaphone_info set album = 'UNKNN' where album = ''") or die("update mp3_metaphone_info set album error: ".mysql_error($this->db_link_id));
			}
			
			public function AlbumUnknow2KnownHTTP()
			{
				$result = mysql_query("select id from mp3_info where filepath like 'http://%' and album = 'Unknown'") or die("select id from mp3_info error: ".mysql_error($this->db_link_id));
				
				$bResult = mysql_query("update mp3_info set album = UCFIRST(SUBSTRING_INDEX(SUBSTRING_INDEX(filepath, '/', -2), '/', 1)) where filepath like 'http://%' and album = 'Unknown'") or die("update mp3_info set album error: ".mysql_error($this->db_link_id));
				
				$index = 0 ;
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					$inner_result = mysql_query("select album from mp3_info where id='".$row['id']."'") or die("select id from mp3_info error: ".mysql_error($this->db_link_id));
					
					if(mysql_num_rows($inner_result) > 0)
					{
						echo "(".$index++.") - ".ucfirst($inner_row['album']) ;
						
						$inner_row = mysql_fetch_array($inner_result, MYSQL_ASSOC) ;
						
						$bResult = mysql_query("update mp3_metaphone_info set album = '".CUtils::GetMetaphone($inner_row['album'])."' where id ='".$row[id]."'") or die("update mp3_metaphone_info set album error: ".mysql_error($this->db_link_id)) ;
					}

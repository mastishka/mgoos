<?php
	...
	class CSearchHelper
	{
		...
		public function PrepareWildQuery($wordArray)
		{
			...
			$sqlQuery = "select mp3_info.id, mp3_info.artist, mp3_info.album, mp3_info.title, mp3_info.year, mp3_info.duration_sec, mp3_info.genre, mp3_info.composer, mp3_info.picturizedon, mp3_info.lyrics, mp3_info.provider, mp3_info.user_id, mp3_info.filepath, (" ;
			$whereClause = "where (" ;
			...
			{
				$sqlQuery = "select id, filepath, artist, album, title, year, time, genre, composer, lyrics, provider, user_id from mp3_info order by title asc" ;
			}
		...
		{
			$query = "SELECT DISTINCT id, artist, album, title, year, duration_sec, genre, composer, picturizedon, lyrics, provider, user_id, filepath, count( mp3_info.id ) AS plays FROM mp3_info, mp3_play_location WHERE (mp3_info.id = mp3_play_location.mp3_id) AND (mp3_play_location.play_datetime = NOW()) GROUP BY id ORDER BY plays DESC;" ;
		...
		{
			$query = "select id, artist, album, title, year, duration_sec, genre, composer, picturizedon, lyrics, provider, filepath, user_id from mp3_info where plays <> 0 ORDER BY plays DESC;" ;
		...
		{
			$query = "select id, artist, album, title, year, duration_sec, genre, composer, picturizedon, lyrics, provider, filepath, user_id from mp3_info where ".$ext."='".$query."';" ;
		...
		public function showUptoTenRecords($query, $page, $ext, $user_id)
		{
			...
			while($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				if($index>=($page - 1)*10 && $index<(($page - 1)*10)+10)
				{
					...
					/*if(strcmp($provider,"http://www.songs.pk") == 0)
					{
						//echo "Test 1 - ".$row["user_id"]."<BR/>";
						$provider_res = mysql_query("select mp3_url from songs_pk_mp3s where id='".$row["user_id"]."'", $this->db_link_id) ;

<?php
	...
	function InsertIntoMp3Info(CId3Info $id3info,$flag)
	{
	...
	$links=mysql_query("select * from crawled_mp3_urls where satus=0",$db_link_id) or die("Sql select error :- ".mysql_error($db_link_id));	
	$num_rows = mysql_num_rows($links);
	while($row = mysql_fetch_assoc($links))
	{
		$url=$row['url'];
		mysql_query("update crawled_mp3_urls set satus= 1 where url like '$url' ",$db_link_id) or die("Sql update error :- ".mysql_error($db_link_id));
		$file=fopen($url,"rb");
		...
		mysql_query("insert into mp3_hphonetic_info values('$met_id','$met_title','$met_album','$met_artist','$met_genre','$met_composer','$met_picturizedon','$met_lyrics')") or die("metaphone insert error :- ".mysql_error($db_link_id));
		
		echo "<pre>";
		echo "added metaphone info of:- ".$row['url']; 
		echo "</pre>";
		ob_flush();
		flush();
	}

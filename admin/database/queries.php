<?php
	...
	class CQueryManager
	{
		...
		{
			$id 			= $id3info->GetID() ;
			$filepath 		= $id3info->GetFilePath() ;
			$filesize 		= $id3info->GetFileSize() ;
			$bitrate 		= $id3info->GetBitRate() ;
			$duration_sec 	= $id3info->GetDurationSec() ;
			$language		= $id3info->GetLanguage() ;
			$status		 	= $id3info->GetStatus() ; // 0 : Private, 1 : Public, 2: Issued Private
			$userid 		= $id3info->GetUserId() ;						
			...
			$bResult = mysql_query("insert into mp3_info (id, upload_date, filepath, filesize, bitrate, duration_sec, language, status, user_id, title, artist, album, lyrics, year, genre,mood, composer, picturizedon) values ('".$id."', CURDATE(), '".$filepath."', '".$filesize."', '".$bitrate."', '".$duration_sec."', '".$language."', '".$status."', '".$userid."', '".$title."', '".$artist."', '".$album."', '".$lyrics."', '".$year."', '".$genre."','".$mood."', '".$composer."', '".$picturizedon."') ;", $this->db_link_id) or die("mp3_info Insert Error: ".mysql_error($this->db_link_id));
		...
		public function PrepareLangOptions($lang)
		{
			...
			while($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				...
				{
					printf("<option value=\"%s\">%s</option>", $row["language"], $row["language"]);
				}
				else
				{
					printf("<option value=\"%s\" selected=\"selected\">%s</option>", $row["language"], $row["language"]);
				}
			}
			mysql_free_result($result);
		}
		...
		public function PrepareGenreOptions($genre)
		{
			...
			while($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				...
				{
					printf("<option value=\"%s\">%s</option>", $row["genre"], $row["genre"]);
				}
				else 
				{
					printf("<option value=\"%s\" selected=\"selected\">%s</option>", $row["genre"], $row["genre"]);
				}
			}
			mysql_free_result($result);
		}
		...
		public function PrepareMoodOptions($mood)
		{
			...
			while($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				...
				{
					printf("<option value=\"%s\" selected=\"selected\">%s</option>", $row["mood"], $row["mood"]);
				}
			}
			mysql_free_result($result);		

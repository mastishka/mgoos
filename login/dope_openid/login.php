	...
	*/
	...
	if($_GET['from']==1)
	{
		if ( ! isset($error)) 
		{
			...
			*/
			$link=CMGooSConfig::MGOOS_ROOT.'/login/dope_openid/login.php?action=verify';
			...
			*/
			$root_link=CMGooSConfig::MGOOS_ROOT;
...
if(strcmp($_GET['openid_mode'],"cancel") == 0)
{
	...
	echo "<script> top.location.href='http://beta.mgoos.com/my_mgoos.php'</script>";
}
	...
	*/
	...
	if ($validate_result === TRUE) 
	{
		...
		mysql_query("insert ignore into alien_users (user_id,email,fname,lname) values ('$userid','$final','$fname','$lname')",$db_link_id) or die ("Could not insert into alien_users: " . mysql_error());
		$link=mysql_query("select user_id from alien_users where email like '$final'",$db_link_id);
		...
		echo "<script> top.location.href='http://beta.mgoos.com/search_results.php'</script>";
	}
